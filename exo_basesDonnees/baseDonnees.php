...
<body>
    ...
    $sql = "SELECT code, villeDepart, villeDestination From trains"; // le texte qu'on va envoyer au server
    $objRequete = $db->prepare($sql);
    $objRequete->execute(); //on lance la requete sur le serveur
    //transformer dans un array
    $arrayResultat = $objRequete->fetchAll(PDO::FETCH_ASSOC); //fetch_assoc - est une constante, de quel
